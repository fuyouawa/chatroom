...
public:
    ...
    auto& GetLoggedSession(uint32_t user_id) const { return logged_session_map_.at(user_id).first; }
    auto& GetLoggedUserInfo(uint32_t user_id) const { return logged_session_map_.at(user_id).second; }
